...
export const oncallReminderSms = onSchedule(
  ...
  async () => {
    ...
    if (!assignSnap.empty) {
      ...
      console.log(`[oncallReminder] Using Firestore override: ${name}`);
    } else if (outlookOnCall.size > 0) {
      onCallNames = new Set([...outlookOnCall]);
      console.log(`[oncallReminder] Using Outlook calendar: ${[...outlookOnCall].join(", ")}`);
    } else {
      const rotPerson = await getRotationOnCallName(today);
      onCallNames = rotPerson ? new Set([rotPerson]) : new Set();
      console.log(`[oncallReminder] Using rotation formula: ${rotPerson || "none"}`);
    }
