...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "vehicles"), (snap) => {
      const rows = snap.docs.map((d) => ({ id: d.id, ...d.data() } as Vehicle));
      const now = new Date();
      rows.sort((a, b) => {
  ...
  const filtered = vehicles.filter((v) => {
    // Status filter
    const isOverdue = v.status === "CHECKED_OUT" && v.dueBackAt?.toDate?.() < now;
    if (filter === "OVERDUE"     && !isOverdue) return false;
    if (filter === "CHECKED_OUT" && !(v.status === "CHECKED_OUT" && !isOverdue)) return false;
    if (filter === "IN_SHOP"     && v.status !== "IN_SHOP") return false;
    if (filter === "DAMAGED"     && v.status !== "DAMAGED") return false;

    // Search filter — name, ID, job number, or employee
    if (q) {
      const nameMatch = (v.name                      ?? "").toLowerCase().includes(q);
      const idMatch   = (v.vehicleId                 ?? "").toLowerCase().includes(q);
      const jobMatch  = (v.checkedOutToJobName        ?? "").toLowerCase().includes(q);
      const empMatch  = (v.checkedOutToEmployeeName   ?? "").toLowerCase().includes(q);
  ...
  function exportCSV() {
    ...
    const rows = filtered.map((v) => {
      ...
      return {
        Name: v.name || "",
        ID: v.vehicleId || v.id,
        Status: isOverdue ? "OVERDUE" : v.status || "",
        Employee: v.checkedOutToEmployeeName || "",
        Job: v.checkedOutToJobName || "",
        "Due Date": fmtDateLong(v.dueBackAt),
      };
    });
    downloadCSV(rows, `vehicles-${filter.toLowerCase()}-${today}.csv`);
  }
  ...
  async function bulkReturn() {
    const checkedOutIds = [...selected].filter((id) => {
      ...
      return vehicle?.status === "CHECKED_OUT";
    });
    if (checkedOutIds.length === 0) {
      toast("No checked-out vehicles selected", "error");
      return;
    }
    if (!await confirm(`Return ${checkedOutIds.length} vehicle(s) to the shop?`)) return;
    ...
    for (const id of checkedOutIds) {
      try {
        ...
        await addDoc(collection(db, "vehicles", id, "history"), {
          action: "RETURNED",
          employeeName: vehicle.checkedOutToEmployeeName ?? "",
          jobName: vehicle.checkedOutToJobName ?? "",
          customer: vehicle.checkedOutToCustomer ?? "",
          recordedAt: serverTimestamp(),
        });
        succeeded++;
      } catch (e) {
        console.error(`Failed to return vehicle ${id}:`, e);
      }
    }
    toast(`Returned ${succeeded} of ${checkedOutIds.length} vehicles`, "success");
    setSelected(new Set());
    setSelectMode(false);
  ...
  const counts = {
    ...
    CHECKED_OUT: vehicles.filter((v) => v.status === "CHECKED_OUT" && !(v.dueBackAt?.toDate?.() < now)).length,
    OVERDUE:     vehicles.filter((v) => v.status === "CHECKED_OUT" && v.dueBackAt?.toDate?.() < now).length,
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        <h1 style={styles.pageTitle}>Shop Vehicles</h1>
        <Link to="/vehicles/new" style={styles.addBtn}>+ Add Vehicle</Link>
      </div>
          ...
          <table style={styles.table}>
            <thead>
              <tr>
                ...
                {["", "Vehicle Name", "ID", "Status", "Employee", "Job", "Check Out Date", "Due Date"].map((h) => (
            ...
            <tbody>
              {filtered.map((v) => {
                ...
                const displayStatus = isOverdue ? "OVERDUE" : v.status ?? "UNKNOWN";
                ...
                return (
                  <tr key={v.id} style={rowStyle}>
                    ...
                    <td style={{ ...styles.td, width: 52, padding: "8px 6px 8px 14px" }}>
                      {v.photoURL ? (
                        <img src={v.photoURL} alt="" onClick={() => setLightboxUrl(v.photoURL!)} style={{ width: 44, height: 44, borderRadius: 6, objectFit: "contain", border: "1px solid #e5e5e5", background: "#fff", cursor: "pointer" }} />
                      ) : (
                    ...
                    <td style={styles.td}>
                      <Link to={`/vehicles/${encodeURIComponent(v.id)}`} style={styles.idLink}>
                        {v.vehicleId || v.id}
                      </Link>
                    </td>
                    <td style={styles.td}><StatusBadge status={displayStatus} /></td>
                    <td style={styles.td}>{v.checkedOutToEmployeeName || "—"}</td>
                    <td style={styles.td}>{v.checkedOutToJobName || "—"}</td>
                    <td style={styles.td}>{fmtDateLong(v.checkedOutAt)}</td>
                    <td style={{ ...styles.td, ...(isOverdue ? { color: "#a80000", fontWeight: 700 } : {}) }}>
                      {fmtDateLong(v.dueBackAt)}
                    </td>
                  </tr>
                );
              })}
