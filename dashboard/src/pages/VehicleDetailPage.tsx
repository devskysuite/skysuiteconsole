...
import type { Vehicle, HistoryEntry, Booking, MaintenanceEntry } from "../types";
...
export default function VehicleDetailPage() {
  ...
  // Per-vehicle rental rate editing
  ...
  async function loadHistory() {
    ...
    try {
      const q = query(collection(db, "vehicles", vehicleId, "history"), orderBy("recordedAt", "desc"));
  ...
  async function loadMaintenanceLogs() {
    ...
    try {
      const q = query(collection(db, "vehicles", vehicleId, "maintenance"), orderBy("date", "desc"));
      const snap = await getDocs(q);
  ...
  async function addMaintenanceLog() {
    ...
    setSaving(true);
    try {
      await addDoc(collection(db, "vehicles", vehicleId, "maintenance"), {
        date: maintDate,
  ...
  async function deleteMaintenanceLog(entryId: string) {
    ...
    try {
      await deleteDoc(doc(db, "vehicles", vehicleId, "maintenance", entryId));
      await loadMaintenanceLogs();
    } catch (e: any) {
  ...
  async function saveInspectionDate() {
    if (!vehicleId || !inspectionDate) return;
    setSavingInspection(true);
    try {
      await updateDoc(doc(db, "vehicles", vehicleId), { lastInspectionDate: inspectionDate });
  ...
  useEffect(() => {
    ...
    // Live listener for vehicle document
    const unsubVehicle = onSnapshot(doc(db, "vehicles", vehicleId), (snap) => {
      if (!snap.exists()) { setVehicle(null); setLoading(false); return; }
    ...
    const bookingsQuery = query(collection(db, "vehicles", vehicleId, "bookings"), orderBy("startDate", "asc"));
  ...
  const now = new Date();
  const isOverdue = isCheckedOut && vehicle?.dueBackAt?.toDate?.() < now;
  const displayStatus = isOverdue ? "OVERDUE" : status;

  async function saveName() {
    if (!vehicleId || !newName.trim()) return;
    setSaving(true);
    try { await updateDoc(doc(db, "vehicles", vehicleId), { name: newName.trim() }); setEditingName(false); }
  ...
  async function checkout() {
    ...
    try {
      ...
      await addDoc(collection(db, "vehicles", vehicleId, "history"), {
        action: "CHECKED_OUT",
  ...
  async function returnVehicle(excludeWeekends: boolean) {
    if (!vehicleId) return;
    const empName    = vehicle?.checkedOutToEmployeeName ?? "";
    const jobName    = vehicle?.checkedOutToJobName ?? "";
    const custName   = vehicle?.checkedOutToCustomer ?? "";
    const checkedOut = vehicle?.checkedOutAt?.toDate?.() as Date | undefined;
  ...
  async function extendDueDate() {
    if (!vehicleId || !extendDate) { setError("Choose a new due date."); return; }
    setSaving(true); setError("");
    try {
      await updateDoc(doc(db, "vehicles", vehicleId), {
  ...
  async function reportDamage() {
    if (!vehicleId || !damageNote.trim()) { setError("Please describe the damage."); return; }
    setSaving(true); setError("");
    try {
      let photoUrl = "";
      if (damagePhoto) {
        const photoRef = ref(storage, `vehicles/${vehicleId}/damage-photo`);
  ...
  async function uploadDamagePhoto(file: File) {
    ...
    try {
      ...
      await updateDoc(doc(db, "vehicles", vehicleId), { damagedPhotoUrl: url });
    } catch (e: any) {
  ...
  async function markRepaired() {
    if (!vehicleId || !await confirm("Mark this vehicle as repaired and return it to the shop?")) return;
    setSaving(true); setError("");
    try {
      await updateDoc(doc(db, "vehicles", vehicleId), {
        status: "IN_SHOP",
  ...
  async function updateRepairStatus(val: string) {
    ...
    try { await updateDoc(doc(db, "vehicles", vehicleId), { repairStatus: val }); }
    catch (e: any) { setError(e?.message); }
  }

  async function saveVehicleRates() {
    if (!vehicleId || !isOwner) return;
    ...
    setSaving(true); setError("");
    try {
      await updateDoc(doc(db, "vehicles", vehicleId), { dayRate: d, weekRate: w, monthRate: m });
  ...
  async function createBooking() {
    if (!vehicleId || !bookEmployee.trim() || !bookJob.trim() || !bookStartDate || !bookEndDate) {
    ...
    try {
      await addDoc(collection(db, "vehicles", vehicleId, "bookings"), {
  ...
  async function cancelBooking(bookingId: string) {
    if (!vehicleId) return;
    try {
      await updateDoc(doc(db, "vehicles", vehicleId, "bookings", bookingId), { status: "CANCELLED" });
    } catch (e: any) { setError(e?.message); }
  }
  ...
  async function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const storageRef = ref(storage, `vehicles/${vehicleId}/photo`);
      await uploadBytes(storageRef, file);
      const url = await getDownloadURL(storageRef);
      await updateDoc(doc(db, "vehicles", vehicleId), { photoURL: url });
  ...
  async function handlePhotoDelete() {
    if (!vehicleId) return;
    if (!await confirm("Remove this vehicle photo?")) return;
    await updateDoc(doc(db, "vehicles", vehicleId), { photoURL: "" });
    setVehicle((prev) => prev ? { ...prev, photoURL: "" } : prev);
  }
  ...
  if (!vehicle) return (
    <div>
      ...
      <button style={s.btnOutline} onClick={() => navigate("/vehicles")}>← Back to Vehicles</button>
    </div>
  );

  const qrValue = vehicle.vehicleId || vehicleId || "";
  const qrName  = vehicle.name || "Unnamed Vehicle";
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  return (
    <>
      ...
      <div>
        <button style={s.backBtn} onClick={() => navigate("/vehicles")}>← Back to Vehicles</button>
        ...
        <div style={s.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
            <div style={{ flex: 1 }}>
              {editingName ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <h2 style={s.toolName}>{vehicle.name || "Unnamed Vehicle"}</h2>
                  <button style={s.editBtn} onClick={() => setEditingName(true)}>✏ Edit</button>
                </div>
              )}
              <p style={s.toolId}>ID: {vehicle.vehicleId || vehicleId}</p>
              <div style={{ marginTop: 10 }}><StatusBadge status={displayStatus} /></div>

              {/* ── Model / Serial # / Notes ── */}
              {(vehicle.model || vehicle.serialNumber || vehicle.notes || isAdmin) && (
                    ...
                    <div style={{ background: "#f8f9fa", borderRadius: 10, padding: 14, border: "1px solid #e5e5e5" }}>
                      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                        <div style={{ flex: "1 1 150px" }}>
                          ...
                          <input style={s.input} value={editModel} onChange={(e) => setEditModel(e.target.value)} placeholder="e.g. Ford F-150" />
                        </div>
                        <div style={{ flex: "1 1 150px" }}>
                          <label style={s.label}>Serial #</label>
                          <input style={s.input} value={editSerial} onChange={(e) => setEditSerial(e.target.value)} placeholder="e.g. VIN-12345" />
                        </div>
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-end" }}>
                      {vehicle.model && (
                        <div>
                          ...
                          <p style={{ margin: "2px 0 0", fontSize: 14, fontWeight: 600, color: "#222" }}>{vehicle.model}</p>
                        </div>
                      )}
                      {vehicle.serialNumber && (
                        <div>
                          <span style={{ color: "#888", fontSize: 12, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: 0.4 }}>Serial #</span>
                          <p style={{ margin: "2px 0 0", fontSize: 14, fontWeight: 600, color: "#222" }}>{vehicle.serialNumber}</p>
                        </div>
                      )}
                      {vehicle.notes && (
                        <div style={{ flex: "1 1 200px" }}>
                          <span style={{ color: "#888", fontSize: 12, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: 0.4 }}>Notes</span>
                          <p style={{ margin: "2px 0 0", fontSize: 14, color: "#555" }}>{vehicle.notes}</p>
                        </div>
                      )}
                      {isAdmin && (
                        <button style={s.editBtn} onClick={() => { setEditModel(vehicle.model ?? ""); setEditSerial(vehicle.serialNumber ?? ""); setEditNotes(vehicle.notes ?? ""); setEditingDetails(true); }}>
                          {vehicle.model || vehicle.serialNumber || vehicle.notes ? "✏ Edit Details" : "+ Add Details"}
                        </button>
                      )}
                    </div>
                  )}
            ...
            {/* ── Vehicle Photo (compact, next to QR) ── */}
          ...
          {showHistory && (
            <div style={s.historyPanel}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 14 }}>
                <h3 style={{ ...s.sectionTitle, marginBottom: 0 }}>Vehicle History</h3>
                {history.length > 0 && (
                  <input
                  ...
                  {filtered.map((h) => {
                    ...
                    return (
                      <div key={h.id} style={{ ...s.timelineItem, background: style.bg, borderLeft: `3px solid ${style.color}` }}>
                        ...
                        {isAdmin && h.action === "RETURNED" && h.daysOnJob !== undefined && vehicle.dayRate !== undefined && (() => {
                          const vehicleRates = { dayRate: vehicle.dayRate!, weekRate: vehicle.weekRate ?? 0, monthRate: vehicle.monthRate ?? 0 };
                          const amount = calcRental(h.daysOnJob, vehicleRates);
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              ...
              {isOwner && !editingRates && (
                <button style={s.editBtn} onClick={() => {
                  setEditDayRate(String(vehicle?.dayRate ?? ""));
                  setEditWeekRate(String(vehicle?.weekRate ?? ""));
                  setEditMonthRate(String(vehicle?.monthRate ?? ""));
                  setEditingRates(true);
                }}>✏ Edit</button>
              )}
            </div>
            {!editingRates ? (
              vehicle?.dayRate === undefined ? (
                <p style={{ color: "#aaa", fontSize: 13, margin: 0 }}>No rates set for this vehicle yet.{isOwner ? " Click Edit to configure." : ""}</p>
              ) : (
                <div style={s.infoGrid}>
                  <span style={s.infoLabel}>Day rate</span>
                  <span>${vehicle.dayRate.toLocaleString()} / day &nbsp;<span style={{ color: "#888", fontSize: 12 }}>(1–4 days)</span></span>
                  <span style={s.infoLabel}>Week rate</span>
                  <span>${vehicle.weekRate?.toLocaleString()} / week &nbsp;<span style={{ color: "#888", fontSize: 12 }}>(5–15 days)</span></span>
                  <span style={s.infoLabel}>Month rate</span>
                  <span>${vehicle.monthRate?.toLocaleString()} / month &nbsp;<span style={{ color: "#888", fontSize: 12 }}>(16+ days)</span></span>
                </div>
              )
            ) : (
              <>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
                  <button style={s.btn} onClick={saveVehicleRates} disabled={saving}>{saving ? "Saving…" : "Save Rates"}</button>
        ...
        {isDamaged && (
          <div style={{ ...s.card, borderColor: "#9c27b0", backgroundColor: "#fdf6ff" }}>
            <div style={s.damagedBanner}>🔧 THIS VEHICLE IS DAMAGED</div>
            ...
            <div style={s.infoGrid}>
              {vehicle.damagedReportedBy && (<><span style={s.infoLabel}>Reported by</span><span>{vehicle.damagedReportedBy}</span></>)}
              <span style={s.infoLabel}>Date reported</span><span>{fmtDateLongWithYear(vehicle.damagedReportedAt)}</span>
              {vehicle.damagedNote && (<><span style={s.infoLabel}>Description</span><span>{vehicle.damagedNote}</span></>)}
            </div>
            {vehicle.damagedPhotoUrl ? (
              <div style={{ marginTop: 16 }}>
                <a href={vehicle.damagedPhotoUrl} target="_blank" rel="noreferrer">
                  <img src={vehicle.damagedPhotoUrl} alt="Damage photo"
        ...
        {isCheckedOut && (
          <div style={{ ...s.card, ...(isOverdue ? s.cardOverdue : {}) }}>
            {isOverdue && <div style={s.overdueBanner}>⚠ THIS VEHICLE IS OVERDUE</div>}
            <h3 style={s.sectionTitle}>Checked Out</h3>
            <div style={s.infoGrid}>
              <span style={s.infoLabel}>Employee</span><span>{vehicle.checkedOutToEmployeeName || "—"}</span>
              <span style={s.infoLabel}>Job Number</span><span>{vehicle.checkedOutToJobName || "—"}</span>
              {vehicle.checkedOutToCustomer && (<><span style={s.infoLabel}>Customer</span><span>{vehicle.checkedOutToCustomer}</span></>)}
              <span style={s.infoLabel}>Checked out</span><span>{fmtDateLongWithYear(vehicle.checkedOutAt)}</span>
              <span style={s.infoLabel}>Due back</span>
              <span style={isOverdue ? { color: "#a80000", fontWeight: 700 } : {}}>{fmtDateLong(vehicle.dueBackAt)}</span>
              {isAdmin && vehicle.dayRate !== undefined && vehicle.checkedOutAt?.toDate && (() => {
                const vehicleRates = { dayRate: vehicle.dayRate!, weekRate: vehicle.weekRate ?? 0, monthRate: vehicle.monthRate ?? 0 };
                const days = countDays(vehicle.checkedOutAt.toDate(), new Date(), false);
                const amount = calcRental(days, vehicleRates);
                  ...
                  return (
                    <>
                      <p style={{ margin: "0 0 10px", fontSize: 14, fontWeight: 600 }}>
                        {days !== null
                          ? `This vehicle was on the job for ${days} day${days !== 1 ? "s" : ""}${returnExcludeWeekends ? " (weekdays only)" : ""}.`
                          : "Confirm return"}
                      </p>
                      {isAdmin && vehicle.dayRate !== undefined && days !== null && (() => {
                        const vehicleRates = { dayRate: vehicle.dayRate!, weekRate: vehicle.weekRate ?? 0, monthRate: vehicle.monthRate ?? 0 };
                        const amount = calcRental(days, vehicleRates);
                        const tier = days <= 4 ? "day rate" : days <= 15 ? "week rate" : "month rate";
                        return (
                      ...
                      <div style={{ display: "flex", gap: 10 }}>
                        <button style={{ ...s.btn, fontSize: 13, padding: "8px 18px" }} onClick={() => returnVehicle(returnExcludeWeekends)} disabled={saving}>
        ...
        {/* ── ANNUAL INSPECTION ── */}
        {(() => {
          const last = vehicle.lastInspectionDate;
          const nextDue = last
                ...
                <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "flex-start" }}>
                  ...
                  <div style={{ flex: "2 1 260px" }}>
                    ...
                    <textarea
                      ...
                      placeholder="e.g. Cracked windshield, dented fender, engine warning light…"
                    />
                  ...
                  {/* Right: repair contacts — filtered by "Vehicles" category */}
                  {(() => {
                    const toolCat = "Vehicles";
